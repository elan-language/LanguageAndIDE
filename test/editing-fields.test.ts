...
suite("Editing Fields Tests", () => {
  ...
  test("Ensure Html tag in a comment is not recognised - #840", () => {
    ...
    assert.equal(
      field.renderAsHtml(),
      `<el-field id="comment2" class="selected focused optional ok" tabindex=0><el-txt><input spellcheck="false" data-cursor="3" size="2" style="width: 3ch" value="<p>"></el-txt><el-place>comment</el-place><completion></completion><msg></msg><help title="Any text on a single line.">?</help></el-field>`,
    );
    field.processKey(tab());
    assert.equal(
      field.renderAsHtml(),
      `<el-field id="comment2" class="optional ok" tabindex=0><el-txt>&lt;p&gt;</el-txt><el-place>comment</el-place><completion></completion><msg></msg><help title="Any text on a single line.">?</help></el-field>`,
    );
  });
