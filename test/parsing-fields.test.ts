...
suite("Field Parsing Tests", () => {
  test("parse CommentField", () => {
    ...
    assert.equal(
      text.renderAsHtml(),
      `<el-field id="comment4" class="optional ok" tabindex=0><el-txt>Hello</el-txt><el-place>comment</el-place><completion></completion><msg></msg><help title="Any text on a single line.">?</help></el-field>`,
    );
  });
