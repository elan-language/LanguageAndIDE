...
const global = new class {};
async function main() {
  await system.printLine(isFavourite(Fruit.apple));
  await system.printLine(isFavourite(Fruit.pear));
}
...
const global = new class {};
async function main() {
  await system.printLine(system.objectEquals(firstFruit(), Fruit.apple));
}
