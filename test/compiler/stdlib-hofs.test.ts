...
main
  print source.filter(lambda x as Int => x > 20).listImmutableAsArray()
  print source.filter(lambda x as Int => x > 20).listImmutableAsArray()
  print source.filter(lambda x as Int => (x < 3) or (x > 35)).listImmutableAsArray()
end main`;
...
async function main() {
  await system.printLine(_stdlib.listImmutableAsArray((await global.source.filter(async (x) => x > 20))));
  await system.printLine(_stdlib.listImmutableAsArray((await global.source.filter(async (x) => x > 20))));
  await system.printLine(_stdlib.listImmutableAsArray((await global.source.filter(async (x) => (x < 3) || (x > 35)))));
}
return [main, _tests];}`;
...
main
  print source.filter(lambda x as String => x is "o").listImmutableAsArray()
end main`;
...
async function main() {
  await system.printLine(_stdlib.listImmutableAsArray((await _stdlib.filter(global.source, async (x) => x === "o"))));
}
return [main, _tests];}`;
...
constant source set to {2, 3, 5, 7, 11, 13, 17, 19, 23, 27, 31, 37}
main
  print source.map(lambda x as Int => x + 1).listImmutableAsArray()
  print source.map(lambda x as Int => x.asString() + "*").listImmutableAsArray()
end main`;
...
async function main() {
  await system.printLine(_stdlib.listImmutableAsArray((await global.source.map(async (x) => x + 1))));
  await system.printLine(_stdlib.listImmutableAsArray((await global.source.map(async (x) => (await _stdlib.asString(x)) + "*"))));
}
return [main, _tests];}`;
...
constant source set to "onetwo"
main
  print source.map(lambda x as String => x + "*").listImmutableAsArray()
end main`;
...
async function main() {
  await system.printLine(_stdlib.listImmutableAsArray((await _stdlib.map(global.source, async (x) => x + "*"))));
}
return [main, _tests];}`;
...
main
  ...
  print source.listImmutableAsArray()
end main`;
...
async function main() {
  ...
  await system.printLine(_stdlib.listImmutableAsArray(source));
}
return [main, _tests];}`;
...
constant source set to {2, 3, 5, 7, 11, 13, 17, 19, 23, 27, 31, 37}
main
  print source.sortBy(lambda x as Int, y as Int => if x is y then 0 else if x < y then 1 else -1).listImmutableAsArray()
  print source
end main`;
...
async function main() {
  await system.printLine(_stdlib.listImmutableAsArray((await global.source.sortBy(async (x, y) => x === y ? 0 : x < y ? 1 : (-1)))));
...
main
  print source.sortBy(lambda x as String, y as String => if x is y then 0 else if x.isAfter(y) then 1 else -1).listImmutableAsArray()
  print source
end main`;
...
async function main() {
  await system.printLine(_stdlib.listImmutableAsArray((await _stdlib.sortBy(global.source, async (x, y) => x === y ? 0 : _stdlib.isAfter(x, y) ? 1 : (-1)))));
  await system.printLine(global.source);
}
return [main, _tests];}`;
...
main
  ...
  print source.listImmutableAsArray().arrayAsSet()
end main`;
...
async function main() {
  ...
  await system.printLine(_stdlib.arrayAsSet(_stdlib.listImmutableAsArray(global.source)));
}
return [main, _tests];}`;
