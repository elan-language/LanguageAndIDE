...
async function main() {
  await system.printLine((await _stdlib.any(global.source, async (x) => x > 20)));
  await system.printLine((await _stdlib.any(global.source, async (x) => (x % 2) === 0)));
  await system.printLine((await _stdlib.any(global.source, async (x) => x > 40)));
}
return [main, _tests];}`;
