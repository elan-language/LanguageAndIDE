...
static (int, int) north((int, int) cell) { // function
  const Int x = cell.item_0;
  const Int y = cell.item_1;
  const Int y2 = if(y == 0, 29, y - 1);
  return (x, y2);
}
...
static int liveNeighbours(List<List<int>> grid, int x, int y) { // function
  ...
  foreach (cell in neighbourCells(x, y)) {
    const Int cx = cell.item_0;
    const Int cy = cell.item_1;
    if (grid[cx][cy] == black) {
      count = count + 1; // set
    }
  }
...
static int nextCellValue(List<List<int>> grid, int x, int y) { // function
  var colour = white;
  const Boolean live = willLive(grid[x][y], liveNeighbours(grid, x, y));
  if (live) {
    colour = black; // set
  }
  return colour;
...
static void nextGeneration(AsRef<List<List<int>>> gridRef) { // procedure
  ...
  foreach (x in range(0, 40)) {
    foreach (y in range(0, 30)) {
      const Int colour = nextCellValue(grid, x, y);
