...
export class EnumValuesField extends AbstractField {
  ...
  override renderAsHtml(): string {
    ...
    if (this.isSelected()) {
      ...
      result = `<el-field id="${this.htmlId}" class="${this.cls()}" tabindex="-1"><el-txt>${textAsHtml}</el-txt><el-place>${this.placeholder}</el-place>${this.getMessage()}${this.helpAsHtml()}</el-field>`;
    }
    return result;
  }
