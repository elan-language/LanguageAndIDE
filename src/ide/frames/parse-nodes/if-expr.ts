...
export class IfExpr extends AbstractSequence {
  ...
  override renderAsHtml(): string {
    return `<el-kw>${ifKeyword} </el-kw>${this.condition?.renderAsHtml()}<el-kw><br>${thenKeyword} </el-kw>${this.whenTrue?.renderAsHtml()}<el-kw><br>${elseKeyword} </el-kw>${this.whenFalse?.renderAsHtml()}`;
  }
}
