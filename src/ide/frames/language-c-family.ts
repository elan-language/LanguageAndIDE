...
export abstract class LanguageCfamily extends LanguageAbstract {
  ...
  common_renderSingleLineAsHtml(frame: Frame): string {
    ...
    if (frame instanceof AssertStatement) {
      ...
      html = `<el-kw>${this.CATCH} (<el-type>Exception</el-type>${frame.variable.renderAsHtml()}) {`;
      ...
      html = `${modifierAsHtml(frame)} ${frame.type.renderAsHtml()} </el-kw>${frame.name.renderAsHtml()};`;
      ...
      html = `<el-kw>${this.ABSTRACT} </el-kw>${frame.type.renderAsHtml()} <el-id>${frame.name.renderAsHtml()}</el-id>;`;
    }
    return html;
  }
  ...
  common_renderTopAsHtml(frame: Frame): string {
    ...
    if (frame instanceof AbstractClass) {
      ...
      html = `<el-kw>${this.FOR} </el-kw>(<el-type>int</el-type> ${v} = ${frame.from.renderAsHtml()}; ${vAsHtml} ${condition} ${frame.to.renderAsHtml()}; ${vAsHtml} = ${vAsHtml} + ${frame.step.renderAsHtml()}) {`;
    } else if (frame instanceof FunctionMethod) {
      html = `${modifierAsHtml(frame)}${frame.returnType.renderAsHtml()} ${frame.name.renderAsHtml()}(${frame.params.renderAsHtml()}) {`;
      ...
      html = `${modifierAsHtml(frame)}<el-kw>${this.VOID} </el-kw>${frame.name.renderAsHtml()}(${frame.params.renderAsHtml()}) {`;
    } else if (frame instanceof TestFrame) {
      html = `<el-kw>${this.STATIC} ${this.VOID} </el-kw>${frame.testName.renderAsHtml()}() {`;
