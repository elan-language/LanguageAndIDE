...
export class LanguagePython extends LanguageAbstract {
  ...
  renderSingleLineAsHtml(frame: Frame): string {
    ...
    if (frame instanceof AbstractFunction) {
      ...
      html = `<el-kw>${this.RAISE}</el-kw>("${frame.text.renderAsHtml()}")`;
  ...
  renderTopAsHtml(frame: Frame): string {
    ...
    if (frame instanceof AbstractClass) {
      ...
      html = `<el-kw>${this.FOR} </el-kw>${frame.variable.renderAsHtml()}<el-kw> ${this.IN} </el-kw><el-method>range</el-method>(${frame.from.renderAsHtml()}, ${frame.to.renderAsHtml()}, ${frame.step.renderAsHtml()}):`;
      ...
      html = `<el-kw>${this.DEF} </el-kw> <el-method>${frame.testName.renderAsElanSource()}</el-method>()-> <el-kw>${this.NONE}</el-kw>:`;
