...
export class AssertStatement extends AbstractFrame implements Statement {
  ...
  renderAsHtml(): string {
    return `<el-statement class="${this.cls()}" id='${this.htmlId}' tabindex="-1" ${this.toolTip()}>${this.contextMenu()}
    <el-method>assertEqual</el-method>(${this.actual.renderAsHtml()}, ${this.expected.renderAsHtml()})
    ${this.helpAsHtml()}${this.compileOrTestMsgAsHtml()}${this.annotationAsHtml()}${this.getFrNo()}</el-statement>`;
  }

  renderAsSource(): string {
    return `${this.indent()}${this.sourceAnnotations()}${assertAnnotation}Equal(${this.actual.renderAsSource()}, ${this.expected.renderAsSource()}) ${this.annotationAsSource()}`;
  }
