...
import { CopyExpr } from "../parse-nodes/copy-expr";
...
export function transform(
  ...
  if (node instanceof CopyExpr) {
    const obj = transform(node.varRef, fieldId, scope) as ExprAsn;
    const changes = transform(node.props, fieldId, scope) as LiteralImmutableListAsn;
