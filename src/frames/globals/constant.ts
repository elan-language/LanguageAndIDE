...
export class Constant extends AbstractFrame implements ElanSymbol, GlobalFrame, Collapsible {
  ...
  renderAsHtml(): string {
    return `<el-const class="${this.cls()}" id='${this.htmlId}' tabindex="0"><top><el-expand>+</el-expand><el-kw>constant </el-kw>${this.name.renderAsHtml()}</top><el-kw> set to </el-kw>${this.value.renderAsHtml()}${this.compileMsgAsHtml()}${this.getFrNo()}</el-const>`;
  }
