...
export class Enum extends AbstractFrame implements ElanSymbol, GlobalFrame, Collapsible {
  ...
  renderAsHtml(): string {
    return `<el-enum class="${this.cls()}" id='${this.htmlId}' tabindex="0"><top><el-expand>+</el-expand><el-kw>enum </el-kw>${this.name.renderAsHtml()}</top> ${this.values.renderAsHtml()}${this.compileMsgAsHtml()}${this.getFrNo()}</enum>`;
  }
