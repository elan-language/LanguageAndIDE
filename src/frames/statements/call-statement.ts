...
export class CallStatement extends AbstractFrame implements Statement {
  ...
  renderAsHtml(): string {
    return `<el-statement class="${this.cls()}" id='${this.htmlId}' tabindex="0"><el-top>${this.contextMenu()}${this.bpAsHtml()}<el-kw>call </el-kw>${this.proc.renderAsHtml()}(${this.args.renderAsHtml()})${this.compileMsgAsHtml()}${this.getFrNo()}</el-statement>`;
  }
