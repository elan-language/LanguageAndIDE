# 727ddc7990241e52f4f3de83124f2d10db6f62f83e69de38f53a3c90ffa1b2fe Elan 1.9.0 guest default_profile valid
...
def strategyA(p: Player, dealerFaceUp: Card) -> Action:  # [function]
  decision = Action.stand # [variable definition]
  drawLimit = 17 # [variable definition]
  if (dealerFaceUp.value >= 4) and (dealerFaceUp.value <= 6) then
    drawLimit = 12 # [assign variable]
  else if p.hasSoftAce then
    drawLimit = 18 # [assign variable]
  if p.handTotal < drawLimit then
    decision = Action.draw # [assign variable]
  return decision
...
def strategyB(p: Player, dealerFaceUp: Card) -> Action:  # [function]
  decision = Action.stand # [variable definition]
  drawLimit = 17 # [variable definition]
  if dealerFaceUp.value is 5 then
    drawLimit = 12 # [assign variable]
  else if p.hasSoftAce then
    drawLimit = 18 # [assign variable]
  if p.handTotal < drawLimit then
