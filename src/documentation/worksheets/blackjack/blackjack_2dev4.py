# 19516e1595ae194860c9abcd389cf9f1c5c139aee9addaa8d3abbd62f7b873df Elan 1.9.0 guest default_profile valid

def main -> None:  # [main]
  points = 10000 # [variable definition]
  rounds = 100000 # [variable definition]
...
def strategyA(p: Player, dealerFaceUp: Card) -> Action:  # [function]
  ...
  dfc = dealerFaceUp.value - 2 # [variable definition]
  limit = 0 # [variable definition]
  if p.hasSoftAce then
    limit = drawToWithSoftAce[dfc] # [assign variable]
  else
    limit = drawTo[dfc] # [assign variable]
  if p.handTotal < limit then
...
def strategyB(p: Player, dealerFaceUp: Card) -> Action:  # [function]
  act = Action.stand # [variable definition]
  if (dealerFaceUp.value >= 4) and (dealerFaceUp.value <= 6) then
    act = drawUpTo(12, p) # [assign variable]
  else if p.hasSoftAce then
    act = drawUpTo(18, p) # [assign variable]
  else
    act = drawUpTo(17, p) # [assign variable]
  return act


def drawUpTo(limit: Int, p: Player) -> Action:  # [function]
  act = Action.stand # [variable definition]
  if p.handTotal < limit then
    act = Action.draw # [assign variable]
  return act
...
drawTo = {17, 17, 12, 12, 12, 17, 17, 17, 17, 17, 17}

drawToWithSoftAce = {18, 18, 12, 12, 12, 18, 18, 18, 18, 12}
